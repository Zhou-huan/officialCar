<template>
  <div class="detail">
    ...
    <div class="user-box f28" v-if="detail.audit_str">
      <div class="user">
          <div class="fC999">审批记录：<span class="fC333 fBold">{{ detail.audit_str }}</span></div>
      </div>
    </div>
...
import { getApplyInfoById, auditRebut, dispatcherApply, getMatchingApplyInfo, getConvoyInfo, cancelApply } from '@/libs/const'
...
export default {
  ...
  mounted () {
    this.$nextTick(() => {
      this.isShowBtn = getPrivilege([100])
      this.getDetail()
      this.getCarpoolList()
      this.getFleetList()
    })
  ...
  methods: {
    ...
    async toOpr (type, state) {
      ...
      if (type === 'confirm' || type === 'dispatch') { // 调度确认 重新调度
        let carpoolIds = []
            ...
            if (res.rows.length) {
              res.rows.some((item) => {
                if (item.operation_state === 1) {
                  this.carpoolList.push({
                    id: item.id,
                    yongNo: item.yong_che_no
                  })
                }
              })
            }
    ...
    async getFleetList () {
      ...
      await getConvoyInfo({
        ...
        if (this.detail.apply_flow_state === 1 && this.detail.apply_convoy_id !== 0 && (this.detail.apply_type === 0 || (this.detail.apply_type === 1 && this.detail.car_sharing_apply_id === 0))) {
          if (!this.$store.state.fleetData.length) {
            if (res.rows.length) {
              res.rows.some((item) => {
                if (item.operation_state === 1) {
                  this.fleetList = [{
